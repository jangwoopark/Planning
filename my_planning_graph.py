...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            node_a = PgNode_a(action) #action node for this action
            if node_a.prenodes.issubset(self.s_levels[level]): #precondition satisfied by previous state level
                a_nodes.append(node_a) #reachable action nodes added to the list of action nodes
                for node_s in self.s_levels[level]:
                    node_s.children.add(node_a) #make this node a child of the preceding level's state node
                    node_a.parents.add(node_s) #the preceding level's state node as parent of this reachable action
        self.a_levels.append(a_nodes) #action level from the reachable action nodes
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level - 1]:
            for node_s in node_a.effnodes: #the effect nodes for this preceding action and add them to the level's state nodes
                s_nodes.add(node_s) #reachable effect node to the set of state nodes
                node_s.parents.add(node_a) #the preceding level's action node as a parent of this reachable effect node
                node_a.children.add(node_s) #this reachable effect node as a child of the preceding action node
        self.s_levels.append(s_nodes) #the state level from the reachable effect nodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect in node_a1.action.effect_add: #node_a1's (+) effects aren't canceled by node_a2's (-) effects
            if effect in node_a2.action.effect_rem:
                return True
        for effect in node_a2.action.effect_add: #node_a2's (+) effects aren't canceled by node_a1's (-) effects
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect in node_a1.action.effect_add: #node_a1's (+) effects don't conflict with node_a2's (-) preconditions
            if effect in node_a2.action.precond_neg:
                return True
        for effect in node_a2.action.effect_add: #node_a2's (+) effects don't conflict with node_a1's (-) preconditions
            if effect in node_a1.action.precond_neg:
                return True
        for effect in node_a1.action.effect_rem: #node_a1's (-) effects don't conflict with node_a2's (+) preconditions
            if effect in node_a2.action.precond_pos:
                return True
        for effect in node_a2.action.effect_rem: #node_a2's (-) effects don't conflict with node_a1's (+) preconditions
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for precond_a1 in node_a1.parents: #node_a1's preconditions aren't mutually exclusive with node_a2's preconditions
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        return node_s1.symbol == node_s2.symbol and node_s1.is_pos != node_s2.is_pos #two state literals don't have opposite polarity
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for precond_s1 in node_s1.parents: #node_s1's preconditions aren't mutually exclusive with node_s2's preconditions
        ...
        for goal in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
                    if goal == state.symbol and state.is_pos: #goal == literal
                        goal_found = True
                        level_sum += level
                        break
                if goal_found:
